...
```{r How has the variability of batting performances changed over seasons? And how does this relate to the mean batting performances?}
db_Battingsd <- db_Batting %>%
  select(yearID, HR) %>%
  group_by(yearID) %>% 
  na.omit() %>%
  mutate(mean = mean(HR)) %>%
...
The configuration of a ballpark and the weather of the city can have an impact on run scoring. How does run scoring vary between ballpark?

Na lang onderzoeken wat de mogelijkheden zijn om deze vraag te kunnen beantwoorden zijn we erachter gekomen dat er geen goede mogelijke mannier is om deze vraag te beantwoorden. Voor deze vraag moet namelijk bekend zijn wat de gebeurtenissen zijn per park, dit is niet te achterhalen met deze dataset. In Teams wordt weergegeven wat het aantal games is van dit team en wat hun homepark is, echter wordt er geen onderschijt gemaakt tussen de uit en thuiswedstrijden. Hierdoor is niet te achterhalen welke bepaalde gebeurtenissen zoals een homerun is voorgekomen in het thuispark of een uitpark. Ook uit de dataset batting kan wel achterhaalt worden welke speler bij welk team hoort en wat dan zijn homepark is, maar ook nu kan er niet achterhaald worden waar de stats van deze speler heeft plaats gevonden, uit of thuis in een ander park.

```{r In soccer it is said that there is a home advantage, can you investigate whether the home advantage does really exist in baseball?}
...
```
Om deze onderzoekvraag te beandwoorden moet er eerst berekend worden wat de win/lose ratio is van een team per jaar per league. Vervolgens moet ok de Thuis/Uit ratio berekend worden per team per jaar per league. Vervolgens moeten deze twee colommen met elkaar vergeleken worden in een correlatie plot. Uit deze correlatieplot is gebleken dat er een correlalatie is van 0 tussen de win/lose ratio en de thuis/uit ratio. Er blijkt dus geen voordeel te zijn voor het thuisspelende team.
Deze onderzoeksvraag had beter en specifieker beantwoord kunnen worden als er een dataset zou zijn waarin de resultaten mer wedstijd waren weergegeven met daarbij welk team thuis speeld. Op deze mannier kan er per wedstrijd vergeleken worden en niet het totale resultaat van één jaar. Dit maakt de uitslag namelijk minder betrouwbaar.

...
```
Na het correct indelen van de classes van elke variabelen kan in het Master tabel de geboorte en overlijdens datum nog beter ingedeeld worden. Dit bereiken wij door de onderstaande code.
```{r Dates fixing}
db_Master$birthDate = as.Date(as.character(paste(db_Master$birthYear,db_Master$birthMonth,db_Master$birthDay,sep = "-")),format = "%Y-%m-%d")
db_Master$deathDate = as.Date(as.character(paste(db_Master$deathYear,db_Master$deathMonth,db_Master$deathDay,sep = "-")),format = "%Y-%m-%d")
```
...
Vervolgens kijken wij naar het salaris van de top 5 spelers volgens een externe bron. Dit betekend niet dat zij de beste spelers zijn statistisch gezien. Maar dit geeft ons wel een referentiepunt.
```{r timeseries 2}
db_Salaries %>% filter(playerID == "ba")
...
percarmaxsal <- mean(aantaljrmaxsalaris$percentage)
```
